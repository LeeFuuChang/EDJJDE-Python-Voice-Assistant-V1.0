...
def GetWeather():
    def WeatherMainWindow():
        ...
        def buildUP():
            """Get API Response"""
            fullOneCalldata = requests.get("https://api.openweathermap.org/data/2.5/onecall?lat={lat}&lon={lon}&lang={lang}&units=metric&appid={API}".format(lat="22.6163",lon="120.3133",lang="zh_TW",API="Your Own OpenWeather API Key"))
            OneCalldata = fullOneCalldata.json()
            if 'Invalid API key' in OneCalldata["message"]:
                raise BaseException("Wrong API , Did you forget to change the OpenWeatherAPI to your own API?")
            fullWeatherdata = requests.get("https://api.openweathermap.org/data/2.5/weather?lat={lat}&lon={lon}&lang={lang}&units=metric&appid={API}".format(lat="22.6163",lon="120.3133",lang="zh_TW",API="Your Own OpenWeather API Key"))
            Weatherdata = fullWeatherdata.json()
            if 'Invalid API key' in Weatherdata["message"]:
                raise BaseException("Wrong API , Did you forget to change the OpenWeatherAPI to your own API?")
...
def sendGmail(SendTo, TheMessage):
    if SendTo == "Your Friends' Gmail":
        raise BaseException("Did you forget to change gmail address to your own friends' ?")
    elif TheMessage:
            ...
            try:
                ...
                try:
                    ...
                    if "寄" in text or "mail" in text or "郵件" in text:
                        ...
                        if "Your Friends Name" in text:
                            sendGmail("Your Friends' Gmail", hearMailMessage())
                    ...
                    if "開" in text and "燈" in text:
                        ...
                        if int(os.system("curl -X POST http://localhost:2004/LightOn"))==0:
                            ...
                            talkTW("我無法連接到電燈開關, 你可能沒有製作Arduino的電器控制, 你可以到github上查看教學, 又或者你並未啟動伺服器")
                        NoCallingTimeOut = 0
                        continue
